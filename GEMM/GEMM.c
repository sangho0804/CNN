...
int main() {
    ...
    //im2col input
    float* gemm_input = out;

    //Once filled with 0. In fact, it gives you a value.
    float weight =
        (float*)calloc(oc * c * k * k, sizeof(float));

    float gemm_output =
        (float*)calloc(h * w * oc, sizeof(float));
